...
var initGoogleMapsQuestion = function (id, container, map) {
    ...
    // Function to set the dataBox to a lat/lng and perform reverse geocoding to get the address
    var setLatLng = function (key, latLng) {
        value[key] = latLng;
        dataBox.value = JSON.stringify(value);
        reverseGeocode(latLng, key);
    };
    // Function to set the dataBox to an address
    var setAddress = function (key, address) {
        value[key] = address;
        dataBox.value = JSON.stringify(value);
    };
...
var reverseGeocode = function (latLng, index) {
    ...
    geocoder.geocode({ 'location': latLng }, function (results, status) {
        if (status === 'OK') {
            if (results[0]) {
                ...
                setAddress(index, 'No results found');
            }
        }
        else {
            setAddress(index, 'Geocoder failed due to: ' + status);
        }
    });
};
