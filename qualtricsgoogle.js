...
var initGoogleMapsQuestion = function (id, container, map) {
    ...
    var setLatLng = function (key, latLng, address) {
        value[key] = { latLng: latLng, address: address };
    ...
    (_a = map.markers) === null || _a === void 0 ? void 0 : _a.forEach(function (marker, index) {
        var _a, _b;
        // Create the marker
        var mapMarker = new google.maps.Marker(__assign(__assign({}, marker.options), { map: googleMap, position: index in value ? value[index].latLng : marker.options.position || map.options.center }));
            ...
            google.maps.event.addListener(locationAutocomplete_1, 'place_changed', function () {
                ...
                if (place.geometry) {
                    ...
                    geocoder.geocode({ 'location': place.geometry.location }, function (results, status) {
                        if (status === 'OK') {
                            if (results[0]) {
                                setLatLng(index, place.geometry.location, results[0].formatted_address);
                            }
                        }
                    });
                }
        ...
        if (draggableMarkerCount === 1) {
            ...
            google.maps.event.addListener(googleMap, 'click', function (event) {
                setLatLng(index, event.latLng, null);
        ...
        google.maps.event.addListener(mapMarker, 'dragend', function (event) {
            ...
            geocoder.geocode({ 'location': event.latLng }, function (results, status) {
                if (status === 'OK') {
                    if (results[0]) {
                        setLatLng(index, event.latLng, results[0].formatted_address);
                    }
                }
            });
        });
